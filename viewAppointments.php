    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      var myEvents = [
        ...
        foreach ($data as $key => $data1) {
          ...
          if ($_SESSION['user'] == $data1['ob'] && $data1['status'] == "Approved") {
            ?> {
              start: '<?php echo $newDate; ?>',
              end: '<?php echo $newDate; ?>',
              title: '<?php echo $data1['usermail']; ?>',
              textColor: 'white'
            },
        <?php
          }
      ...
      var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        header: {
          ...
          eventLimit: true // allow more events 
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="col-md-12">
                <div class="card">
                  <div class="card-header card-header-primary">
                    <h3 class="card-title">Appointments for Today</h3>
                  </div>
                  <div class="card-body">
                      ...
                      <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                          ...
                          foreach ($data as $key => $data1) {
                            $date = str_replace('-', '/', $data1['date']);
                            $newDate = date("Y-m-d", strtotime($date));
                            if ($_SESSION['user'] == $data1['ob'] && $newDate == date("Y-m-d") && $data1['status'] == "Approved") {
                              $i++;
                              ?>
                              <tr>
                                ...
                                <td>
                                  ...
                                  <button type="button" class="btn btn-success" data-toggle="modal" data-target="#appointmodal"><i class="fa fa-plus"></i> Move Appointment</button>
                                  <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#cancelmodal"><i class="fa fa-trash"></i> Cancel</button>
                                    ...
                                    <div class="modal-dialog" role="document">
                                      <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                          <form action="update_appointment.php" method="POST" enctype="multipart/form-data">
                                            <div class="form-group">
                                              <label for="nextAppt">Are you sure you want to cancel the appointment?</label>
                                            </div>
                                            <input type="hidden" name="ref" value="Calendar/<?php echo $key; ?>">
                                            <button type="submit" name="delete" class="btn btn-danger">Yes</button>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">No</span></button>
                                          </form>
                                        </div>
                                      </div>
                                    </div>
